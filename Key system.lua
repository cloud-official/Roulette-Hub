...
--///DISCORD LINK \\\-- 
local KeyLink = "https://yourkeysite.com"
...
local function unlock(level, usedKey)
	...
	print("Execute:", string.format("Tier: %s | Key: %s", level, usedKey))

	-- You can add specific logic:
	if level == "NIGGA SYSTEM2" then
		-- Example: give tools, admin GUI, etc.
		print("GUI ETC BUT BETTER")
	elseif level == "NIGGA SYSTEM" then
		print("GUI ETC")
	end
end
...
getKey.MouseButton1Click:Connect(function()
	if KeyLink and KeyLink ~= "" then
		...
		notify("📋 Copied", "discord copied to clipboard!", 3)
	else
